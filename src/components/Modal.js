    ...
    <>
      ...
      >
        ...
        <ul id='ullocs' style={{ maxHeight: "400px", overflowY: "scroll" }}>
              {modalitems?.map(
                (
                  item, index // ? so maps will only be called if locs aren't either null or undefined
                ) => (
                  <li key={item.ID}>
                    {index === 0 && (
                  
                    <b id='lastloci'>{name} last location:<br /></b>
                    
                  
                )}
                    <b>LocationID: </b> {item.ID}
                    <br />
                    <b>Date:</b> {item.CreatedAt.split("T")[0]}<b>,</b>
                    <b>  Hour:</b> {item.CreatedAt.slice(11, 16)}
                    <br />
                    <b>Latitude:</b> {item.Latitude}<b>, </b>
